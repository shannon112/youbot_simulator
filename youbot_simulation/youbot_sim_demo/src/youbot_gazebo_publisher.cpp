...
trajectory_msgs::JointTrajectory createGripperPositionCommandL(double newPosition) {

	trajectory_msgs::JointTrajectory msg;
	trajectory_msgs::JointTrajectoryPoint point;

	// fill trajectory_msgs::JointTrajectory|trajectory_msgs::JointTrajectoryPoint[] points;
	for (int i = 0; i < 3; i++) {
	...
	// fill trajectory_msgs::JointTrajectory|string[] joint_names
	for (int i = 0; i < 1; i++) {
		std::stringstream jointName;
		if(i==0)
	...
	// fill trajectory_msgs::JointTrajectory|std_msgs/Header header
...
// create a message for the gripper
trajectory_msgs::JointTrajectory createGripperPositionCommandR(double newPosition) {
	...
	// fill trajectory_msgs::JointTrajectory|std_msgs/Header header
	msg.header.frame_id = "gripper_finger_joint_r";
	msg.header.stamp = ros::Time::now();
...
void moveGripper() {
	trajectory_msgs::JointTrajectory msgR;
	trajectory_msgs::JointTrajectory msgL;

	// open gripper//0.011
	msgR = createGripperPositionCommandR(0.08);
	msgL = createGripperPositionCommandL(0.08);
	gripperPublisher.publish(msgR);
	gripperPublisher.publish(msgL);

	ros::Duration(1).sleep();

	// close gripper
	msgR = createGripperPositionCommandR(0);
	msgL = createGripperPositionCommandL(0);
	gripperPublisher.publish(msgR);
	gripperPublisher.publish(msgL);
}
